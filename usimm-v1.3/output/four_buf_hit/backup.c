...
long long int not_column_act = 0;
...
void schedule(int channel)
{
	...
	if(drain_writes[channel])
	{
		...
		{
			buffer_hits[channel][my_wr->dram_addr.rank][my_wr->dram_addr.bank] = 0;
...
*/
	...
	if(!drain_writes[channel])
	{
		...
		if(my_rd && my_rd->next_command == COL_READ_CMD && buffer_hits[channel][my_rd->dram_addr.rank][my_rd->dram_addr.bank] > 3)
		{
			buffer_hits[channel][my_rd->dram_addr.rank][my_rd->dram_addr.bank] = 0;
			//printf("many hits check close\n");
...
void scheduler_stats()
{
  ...
  printf("Number of non column access: %lld\n",not_column_act);
}
