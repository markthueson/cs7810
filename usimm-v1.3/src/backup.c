...
int buf_hits[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];
...
void schedule(int channel)
{
	...
	if(!drain_writes[channel])
	{
		...
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			...
			{
				my_rd = rd_ptr;
				if(buf_hits[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] > 0)
					bufhit = 1;
				break;
			}
		}
		*/
		//for(rd_ptr;rd_ptr && !bufhit;rd_ptr=rd_ptr->next)
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			//if(rd_ptr->command_issuable && buf_hits[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] > 0)
			if(rd_ptr->command_issuable)
			{
				// Before issuing the command, see if this bank is now a candidate for closure (if it just did a column-rd/wr).
				 //  If the bank just did an activate or precharge, it is not a candidate for closure. 
				if (rd_ptr->next_command == COL_READ_CMD) {
				  //if(buf_hits[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] > 3) {
  				    recent_colacc[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = 1;
				  //}
				  //else buf_hits[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = 0;
				}
		...
		//++buf_hits[channel][my_rd->dram_addr.rank][my_rd->dram_addr.bank];
